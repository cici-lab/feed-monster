...
| 大眼仔 | ✅ 已有 | 普通 | 初始可选 |
| 毛毛怪 | ✅ 已有 | 普通 | 初始可选 |
| 火焰小鬼 | 📝 待开发 | 稀有 | 累计投喂100次 或 完成任意传说配方 |
| 影子精灵 | 📝 待开发 | 传说 | 完成全部稀有配方 或 累计分数10000 |

#### 1.2 怪物形象美化

- [ ] 动画帧：眨眼、呼吸起伏、开心跳跃等微动效
- [ ] 表情变化：不同状态有更明显的表情差异
- [ ] 配件系统：帽子、围巾等可解锁装饰

#### 1.3 怪物能力实现（代码已定义但未生效）

| 怪物 | 特殊能力 | 效果描述 |
|---|---|---|
| 大眼仔 | 配方提示 | 在合成炉界面显示"缺XX类食材"提示 |
| 毛毛怪 | 奇怪食物友好 | 投喂奇怪物品不减生命值 |
| 火焰小鬼 | 火焰亲和 | 岩浆/火药类食物 +50% 效果 |
| 影子精灵 | 暗影护盾 | 所有负面效果减半 |
...
#### 2.1 视觉识别强化（解决"看不出是什么食材"的问题）

| 方案 | 描述 | 优先级 |
|---|---|---|
| Emoji 标签 | 在食材上方显示emoji图标（🍔蛋糕、🔋电池） | P0 |
| 名字悬浮显示 | 拖拽时显示食材名称标签 | P0 |
| 独特轮廓形状 | 用多边形替代圆形（蛋糕→三角形、钥匙→锯齿形） | P1 |
| 颜色+图案组合 | 添加内部图案纹理（骨头→骨形图案） | P2 |
...
|---|---|---|---|
| 入门配方 | 5个 | 直接显示 | 无 |
| 普通配方 | 4个 | 显示但食材模糊 | 首次成功合成入门配方 |
| 稀有配方 | 4个 | 仅显示名称+图标 | 完成所有普通配方 |
| 传说配方 | 3个 | 完全隐藏，需要发现 | 完成所有稀有配方 |

#### 3.2 新增入门配方（让玩家快速体验合成）

| 名称 | 食材组合 | 分数 | 效果 |
|---|---|---|---|
| 能量棒 | 树枝 + 电池 | 35 | 小幅恢复饱食度 |
| 自然套餐 | 树叶 + 花朵 + 果汁 | 50 | 连击+1 |
| 脏脏套餐 | 脏水 + 袜子 | 30 | 无特殊效果（搞笑配方） |
| 岩石大餐 | 石头 + 树枝 + 蘑菇 | 45 | 撒下更多食物 |
| 简易药水 | 脏水 + 蘑菇 | 32 | 小幅恢复饱食度 |
...
| 阶段 | 改进项 | 依赖关系 |
|---|---|---|
| **阶段1** | 食材 Emoji 标签/名字显示 | 无 |
| **阶段1** | 新增入门配方 | 无 |
| **阶段2** | 新增火焰小鬼（稀有怪物） | 怪物解锁系统 |
| **阶段2** | 怪物能力实现 | 无 |
| **阶段3** | 新增影子精灵（传说怪物） | 阶段2完成 |
| **阶段3** | 配方解锁进度可视化 | 无 |
| **阶段4** | 成就系统 | 阶段2、3完成 |
| **阶段4** | 怪物养成/羁绊系统 | 阶段4成就系统 |
